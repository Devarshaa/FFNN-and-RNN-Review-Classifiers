...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden_representation, recurring_weights = self.rnn(inputs, torch.tensor(np.zeros([1, 1, self.h]), dtype = inputs.dtype))
        # [to fill] obtain output layer representations
        real_outputs = self.W(hidden_representation[-1])
        # [to fill] sum over output
        sum_outputs = torch.sum(real_outputs, dim = 0)
...
if __name__ == "__main__":
    ...
    plt.xlabel('Epochs')
    plt.xticks(arange(1, epoch + 1, 1))
    plt.yticks(arange(0, 1.2, 0.1))
    plt.legend(loc='best')
    plt.show()
